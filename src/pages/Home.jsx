...
const LiveCodingDemo = ({ currentTheme, githubStats }) => {
  ...
  return (
    <div className={`relative group ${currentTheme === 'dark'
        ...
        rounded-2xl p-4 sm:p-6 lg:p-8 font-mono text-sm border-2 shadow-2xl w-full max-w-none overflow-hidden
        hover:shadow-3xl transition-all duration-500 hover:scale-[1.02] transform-gpu`}>
      <div className="absolute -inset-0.5 bg-gradient-to-r from-teal-500 via-cyan-500 to-teal-500 rounded-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500 animate-pulse"></div>
      <div className="relative">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <span className={`${currentTheme === 'dark' ? 'text-slate-400' : 'text-slate-500'} text-sm font-medium`}>about-me.js</span>
          </div>
        ...
        <div className="relative bg-gradient-to-br from-slate-950 to-slate-900 rounded-lg p-4 sm:p-6 overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-r from-teal-500/5 via-transparent to-cyan-500/5"></div>
          <pre
            className="relative text-teal-400 text-sm sm:text-base leading-relaxed whitespace-pre-wrap break-all font-mono"
            style={{ minHeight: '260px', maxHeight: '260px', overflow: 'hidden' }}
          >
            {code}
...
// --- FIXED EnhancedTechStack with working filters and SVG progress ring ---
const EnhancedTechStack = ({ currentTheme, skills }) => {
  ...
  // Add any extra categories from your data (case-insensitive, unique)
  ...
  // Filtering logic: always compare lowercased and trimmed
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      <div className="flex flex-wrap gap-3 sm:gap-4 mb-12 justify-center px-4">
        {categories.map((category) => (
          <button
            key={category}
            type="button"
            onClick={() => setSelectedCategory(category)}
            className={`px-6 py-3 sm:px-8 sm:py-4 rounded-2xl text-sm sm:text-base font-semibold transition-all duration-400 transform hover:scale-105 ${
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-6 sm:gap-8 w-full px-4">
        {filteredSkills.map((skill, index) => (
          <motion.div
            key={skill.id}
            className={`relative group overflow-hidden rounded-xl p-4 sm:p-5 transition-all duration-300 h-full ${
              currentTheme === 'dark'
          ...
          >
            ...
            <div className="flex flex-col items-center">
              <div className="mb-3 relative">
                {skill.iconUrl ? (
                  <img
                    ...
                    alt={`${skill.name} icon`}
                    className="w-12 h-12 sm:w-14 sm:h-14 rounded-full object-cover border-2 border-teal-500/30"
              ...
              <div className="relative w-16 h-16 sm:w-20 sm:h-20 mt-3 flex items-center justify-center">
        ...
        {projects.map((project, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <h3
                  className={`text-lg font-bold mb-1 ${currentTheme === 'dark' ? 'text-white' : 'text-slate-800'} line-clamp-1`}
                >
                  {project.title}
                </h3>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4">
        ...
        {contactDetails.map((detail, index) => (
          <div key={detail.label} 
            className={`group relative ${currentTheme === 'dark' 
              ...
              p-6 sm:p-8 rounded-2xl border-2 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-500 w-full 
              ${detail.isClickable ? 'hover:scale-105 cursor-pointer transform-gpu hover:-translate-y-1' : 'hover:scale-102'}`}
            style={{ animationDelay: `${index * 100}ms` }}>
            <div className="flex items-start gap-6">
              <div className={`${detail.bgColor} ${detail.borderColor} p-4 rounded-2xl border-2 group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
                <detail.icon className={`w-6 h-6 sm:w-8 sm:h-8 ${detail.color}`} />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className={`font-bold ${currentTheme === 'dark' ? 'text-white group-hover:text-teal-300' : 'text-slate-800 group-hover:text-teal-600'} 
                  text-lg sm:text-xl mb-3 transition-colors duration-300`}>
                  {detail.label}
                </h3>
                <p className={`${currentTheme === 'dark' ? 'text-slate-300' : 'text-slate-700'} text-base sm:text-lg leading-relaxed break-words`}>
                  {detail.isClickable ? (
      ...
      <div className={`${currentTheme === 'dark' 
        ...
        p-8 rounded-2xl border-2 backdrop-blur-sm shadow-xl w-full`}>
        <h3 className={`text-2xl font-bold ${currentTheme === 'dark' ? 'text-white' : 'text-slate-800'} text-center mb-8`}>
          GitHub Statistics
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div className="text-center">
            <div className="text-3xl sm:text-4xl font-bold text-teal-400 mb-2">{githubStats?.totalRepos || 25}</div>
            <div className={`text-sm sm:text-base ${currentTheme === 'dark' ? 'text-slate-400' : 'text-slate-600'} font-medium`}>Repositories</div>
          </div>
          <div className="text-center">
            <div className="text-3xl sm:text-4xl font-bold text-cyan-400 mb-2">{githubStats?.totalStars || 150}</div>
            <div className={`text-sm sm:text-base ${currentTheme === 'dark' ? 'text-slate-400' : 'text-slate-600'} font-medium`}>Total Stars</div>
          </div>
          <div className="text-center">
            <div className="text-3xl sm:text-4xl font-bold text-emerald-400 mb-2">{githubStats?.followers || 45}</div>
...
const Portfolio = () => {
  ...
  // Gradient animation (from Skills.jsx)
  useEffect(() => {
    const interval = setInterval(() => {
  ...
  return (
    <div className={`relative min-h-screen transition-all duration-500 ${currentTheme === 'dark' ? 'bg-slate-900' : 'bg-white'}`}>
      ...
      />
      <div
        className={`absolute -top-20 -right-20 w-64 h-64 bg-teal-500 rounded-full filter blur-3xl ${currentTheme === 'dark' ? 'opacity-10' : 'opacity-5'} animate-pulse`}
      ></div>
      <div
        className={`absolute -bottom-16 -left-16 w-72 h-72 bg-cyan-500 rounded-full filter blur-3xl ${currentTheme === 'dark' ? 'opacity-10' : 'opacity-5'} animate-pulse-slow`}
      ></div>
      <Particles isInView={isVisible['hero'] || isVisible['skills'] || isVisible['projects']} />

      {/* Hero Section */}
      <section id="hero" className="pt-20 sm:pt-24 pb-16 sm:pb-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 sm:mb-16">
            <div className="relative inline-block mb-6 sm:mb-8">
              <div className="w-24 h-24 sm:w-32 sm:h-32 lg:w-40 lg:h-40 bg-gradient-to-r from-teal-500 to-cyan-500 rounded-full mx-auto mb-6 shadow-2xl shadow-teal-500/30 flex items-center justify-center text-4xl sm:text-5xl lg:text-6xl animate-pulse">
                👨‍💻
              </div>
            ...
            <h1 className={`text-4xl sm:text-5xl lg:text-7xl font-bold ${currentTheme === 'dark' ? 'text-white' : 'text-slate-800'} mb-4 sm:mb-6`}>
              Hi, I'm <span className="bg-gradient-to-r from-teal-400 to-cyan-400 bg-clip-text text-transparent">Syed Azadar Hussain</span>
            </h1>
            <p className={`text-xl sm:text-2xl lg:text-3xl ${currentTheme === 'dark' ? 'text-slate-300' : 'text-slate-700'} mb-6 sm:mb-8 max-w-4xl mx-auto leading-relaxed`}>
              Full Stack Developer passionate about creating amazing web experiences with modern technologies
            </p>
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center items-center mb-12 sm:mb-16">
              <a
                href="#contact"
                className="px-8 py-4 bg-gradient-to-r from-teal-500 to-cyan-500 text-white text-lg font-semibold rounded-2xl hover:from-teal-600 hover:to-cyan-600 transition-all duration-300 shadow-xl hover:shadow-2xl hover:scale-105 transform-gpu w-full sm:w-auto text-center"
              >
                Let's Connect 🚀
              </a>
              <a
                href="#projects"
                className={`px-8 py-4 text-lg font-semibold rounded-2xl transition-all duration-300 shadow-xl hover:shadow-2xl hover:scale-105 transform-gpu w-full sm:w-auto text-center ${currentTheme === 'dark' 
      ...
      {/* Journey Section
      <section id="journey" className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className={`text-3xl sm:text-4xl lg:text-5xl font-bold ${currentTheme === 'dark' ? 'text-white' : 'text-slate-800'} mb-4 sm:mb-6`}>
              My Professional <span className="bg-gradient-to-r from-teal-400 to-cyan-400 bg-clip-text text-transparent">Journey</span>
            </h2>
            <p className={`text-lg sm:text-xl ${currentTheme === 'dark' ? 'text-slate-400' : 'text-slate-600'} max-w-3xl mx-auto`}>
              A glimpse into my career path, experiences, and the skills I've acquired
            </p>
          </div>
          <ProfessionalJourney currentTheme={currentTheme} journey={journey} />
        </div>
      </section> */}
